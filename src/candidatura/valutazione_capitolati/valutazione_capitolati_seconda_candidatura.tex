...
\item \textbf{\term{Architettura} Cloud Serverless:} utilizzo di servizi \textbf{AWS} (\term{Api} Gateway, Lambda, DynamoDB, RDS, S3) per realizzare un’infrastruttura scalabile, resiliente e a basso costo di gestione.
...
\item \textbf{Comunicazione tra front-end e back-end:} \term{Api} REST sicure per l’interazione tra app mobile e backend; gestione asincrona tramite \textbf{AWS SQS/Kinesis} e orchestrazione con \textbf{Step Functions}.
...
\begin{itemize}
    ...
    \item \textbf{Interesse tecnico sul grafo di note:} Lo sviluppo della funzionalità di archiviazione e collegamento delle note tramite \term{Database} rappresenta una sfida interessante e aggiunge valore sia in termini di usabilità che di impatto visivo.
