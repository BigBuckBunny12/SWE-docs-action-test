...
\begin{itemize}
    ...
    \item \textbf{Revisione e valutazione}, perché un prodotto entri in \term{Baseline}, le modifiche apportate devono essere prima verificate da un verificatore (\hyperref[sec:processo-\term{Verifica}]{si veda Sezione~\ref*{sec:processo-\term{Verifica}}}) e successivamente revisionate dal responsabile. Quest’ultimo accerta che i prodotti software e i piani risultino conformi ai requisiti stabiliti, valutando l’esito dei compiti completati e garantendo qualità e coerenza con gli obiettivi del processo.
...
\end{itemize}
L'esito di ogni incontro, interno o esterno, deve essere registrato mediante la redazione di un \term{Verbale}, secondo le modalità previste dal processo di documentazione (\hyperref[sec:documentazione]{vedi Sezione~\ref*{sec:documentazione}}). 
