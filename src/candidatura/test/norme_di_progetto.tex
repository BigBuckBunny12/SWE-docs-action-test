...
\hline
\rowcolor{white}
0.11.2 & 2026-01-11 & Dennis Parolin & Aggiunti gli strumenti di supporto delle sezioni "Processo di \term{\term{Verifica}}", "Processo di \term{\term{Validazione}}" e "Codifica". Aggiunta la lista delle tabelle. & Riccardo Manisi \\
\hline
\rowcolor{gray!20}
0.11.1 & 2026-01-10 & Dennis Parolin & Sistemate le sezioni "Processo di \term{\term{Verifica}}" e "Processo di \term{\term{Validazione}}" tra i Processi di Supporto. Sistemata la sezione "Codifica" tra i Processi Primari. & Riccardo Manisi\\
\hline
\rowcolor{white}
0.11.0 & 2026-01-06 & Dennis Parolin & Aggiunta sezioni "Processo di \term{\term{Verifica}}" e "Processo di \term{\term{Validazione}}" tra i Processi di Supporto. Aggiunta la sezione "Codifica" tra i Processi Primari. & Riccardo Manisi \\
\hline
\rowcolor{gray!20}
0.10.1 & 2026-01-06 & Ferdinando Fracasso & Aggiunta "\term{\term{Piano Di Qualifica}}" nella sezione Documentazione fornita  & Riccardo Manisi\\
\hline
\rowcolor{white}
...
Il presente documento è volto a definire i processi del \term{\term{Way Of Working}} per il gruppo BitByBit che si impegna a mantenerlo per tutta la durata del progetto.
...
\begin{itemize}\raggedright
    ...
    \item \textbf{ISO/IEC 12207:1997 - Software \term{\term{Life Cycle}} Processes}\\
    Parti consultate: documento completo\\
...
\begin{itemize}
    ...
    \item \textbf{Revisione e \term{\term{Validazione}}:}
    Il fornitore coordina attività di revisione con il proponente, anche durante lo sviluppo del prodotto poiché necessario per avere feedback per piani futuri, ed effettua le necessarie attività di \term{\term{Verifica}} e \term{\term{Validazione}} per garantire la qualità del prodotto.
...
Inoltre il proponente si rende disponibile a incontri da remoto, tramite Google Meet, o in presenza, quando necessario per la \term{\term{Verifica}} dello stato di avanzamento del progetto.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Documento in cui il gruppo BitByBit ufficializza la propria candidatura al \term{\term{Capitolato}} dell’azienda Miriade S.r.l.
        \\Il documento contiene una versione riassunta delle ragioni per la scelta del \term{\term{Capitolato}}, un resoconto generale degli incontri preliminari effettuati con delle aziende proponenti e informazioni riguardanti il budget e la data di consegna prevista del progetto.
    \end{itemize}
    
    \item \textbf{Valutazione dei capitolati:}
    \begin{itemize}
        \item Documento in cui viene fatta una analisi dettagliata di ciascun \term{\term{Capitolato}} reso disponibile dai proponenti, valutandone punti a favore e punti critici, con maggiore attenzione posta sul \term{\term{Capitolato}} scelto dal gruppo.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \\Questo in modo da avere un punto di riferimento primario per le attività di progettazione, sviluppo, \term{\term{Verifica}} e \term{\term{Validazione}}, e garantire la conformità del prodotto finale alle richieste del proponente.
    \end{itemize}

    \item \textbf{Norme di progetto:}
    \begin{itemize}
        \item Il presente documento, avente l'obiettivo di definire i processi del \term{\term{Way Of Working}} e le norme che il gruppo BitByBit si impegna a seguire per l'intera durata del progetto.
    \end{itemize}

    \item \textbf{\term{\term{Piano Di Progetto}}:}
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Analisi dei requisiti di sistema}, l'analisi del sistema da implementare deve essere condotta in modo dettagliato, al fine di produrre una descrizione delle funzionalità e delle capacità del sistema, nonché dei requisiti \term{\term{Utente}} e di sistema.
    \item \textbf{Progettazione dell'\term{\term{Architettura}}}, deve essere identificata un'\term{\term{Architettura}} di alto livello per identificare gli elementi software. Deve essere garantito che tutti i requisiti siano allocati tra gli elementi.
    ...
    \item \textbf{Installazione del software}, comprende la consegna del software al cliente e la sua installazione nell'ambiente concordato dal \term{\term{Contratto}}.
...
L'attività di analisi dei requisiti ha come obiettivo trasformare i requisiti \term{\term{Utente}} espressi nel \term{\term{Capitolato}}, nel nostro caso il \textbf{\term{\term{Capitolato}} C4} proposto da \textbf{Miriade S.r.l.}, in requisiti di sistema chiari, completi e verificabili. 
...
\begin{itemize}
    \item \textbf{Obbligatori} (\textbf{OB}): necessari e irrinunciabili per la soddisfazione minima degli \term{\term{Stakeholder}};
...
Ogni \term{\term{Requisito}} è identificato tramite una codifica standard:
\begin{center}
    \texttt{R<tipo>-<importanza>-X}
\end{center}
Dove:
\begin{itemize}
    \item \texttt{R}: è per abbreviare la parola \term{\term{Requisito}}
    \item \texttt{<tipo>}: è la sigla che indica la tipologia di \term{\term{Requisito}}
    \begin{itemize}
        \item \texttt{F}: per indicare che è un \term{\term{\term{Requisito} Funzionale}}
        \item \texttt{Q}: per indicare che è un \term{\term{Requisito}} di Qualità
        \item \texttt{V}: per indicare che è un \term{\term{Requisito}} di Vincolo
    \end{itemize}
    \item \texttt{<importanza>}: è la sigla a due lettere che indica la categoria del \term{\term{Requisito}}
    \item \texttt{X}: è un identificatore numerico univoco
\end{itemize}

Tale classificazione supporta la tracciabilità, la \term{\term{Verifica}} e la gestione dell’evoluzione dei requisiti durante l’intero ciclo di vita.
...
Il processo di \term{\term{Verifica}} dello stile avviene su due livelli:
\begin{itemize}
    ...
    \item \textbf{Remoto}: la pipeline di \textbf{GitHub Actions} \term{\term{Verifica}} la conformità del codice ad ogni \textit{push}. Come da policy, il mancato rispetto delle norme di formattazione impedisce tecnicamente il merge delle modifiche nel ramo principale.
\end{itemize}
...
Gli strumenti critici per il mantenimento dello standard, le cui specifiche tecniche verranno consolidate durante la Product \term{\term{Baseline}} (\term{\term{Pb}}), sono classificabili in:
\begin{itemize}
...
\begin{itemize}
    ...
    \item[(d)] \textbf{\term{\term{Verifica}};}
    \item[(e)] \textbf{\term{\term{Validazione}}.}
\end{itemize}
...
\begin{enumerate}
  ...
  \item Assicurare la \textbf{tracciabilità} delle modifiche e delle versioni;
   ...
   \item Fornire un \textbf{mezzo ufficiale di comunicazione} tra il gruppo, il \term{\term{Committente}} e i revisori;
   \item Rispettare la \textbf{uniformità formale e stilistica} tra i vari documenti prodotti.
\end{enumerate}
L’implementazione del \term{\term{Repository}} dedicato alla documentazione integra tutti gli strumenti necessari alla produzione dei documenti di ciclo di vita, garantendo che ogni nuovo membro del progetto possa svolgere le attività previste senza dipendere dal proprio ambiente di lavoro locale o incorrere in limitazioni tecniche.\\È necessaria l'automazione dei processi dove è possibile, in modo da avere un sistema con pipeline per lo sviluppo che aiuti il membro del gruppo nei suoi compiti.

\subsubsection{Strumenti a supporto}
Per sostenere il processo di documentazione sono utilizzati i seguenti strumenti:
\begin{itemize}
  \item \textbf{\LaTeX}, per la stesura di documenti il gruppo ha scelto \LaTeX, linguaggio di markup che permette di avere il controllo di tutte le parti di un documento, può essere eseguito nel \term{\term{Repository}} senza la necessità di un editor di testo; 
  ...
  \item \textbf{GitHub}, piattaforma di versionamento distribuito e piattaforma di collaborazione utilizzata per la gestione centralizzata dei documenti, delle modifiche, dei branch e delle \term{\term{Pull Request}};  
...
\begin{itemize}
    ...
    \item \textbf{Documenti esterni}, destinati a revisori, docenti o aziende (Analisi dei Requisiti, \term{\term{Piano Di Progetto}}, \term{\term{Piano Di Qualifica}});
...
\subsubsection{Implementazione di processo}
Ogni documento, prima di essere integrato nella versione stabile del \term{\term{Repository}}, deve attraversare un ciclo strutturato di produzione che garantisce la qualità, la verificabilità e la tracciabilità delle modifiche. Le attività descritte in questa sezione definiscono il flusso operativo seguito dal gruppo per l’elaborazione, la revisione e la pubblicazione dei prodotti documentali.

\paragraph{Creazione delle issue GitHub}
Ogni azione che porta a dei cambiamenti ad un documento del gruppo deve iniziare con l’apertura di una issue su GitHub per permettere la tracciabilità nella board del GitHub \term{\term{Project}}. Ogni commit deve fare riferimento ad una sola azione in modo da garantire che il processo iterativo di revisione ed eventuali correzioni di errori sia il più veloce possibile.
...
Se le modifiche implicano la creazione del file di documentazione il membro del gruppo è tenuto a creare il file sorgente, denominato secondo le convenzioni stabilite. Una volta inserito il nuovo file nel \term{\term{Repository}} remoto, il sistema provvede automaticamente ad applicare il template comune, garantendo l’integrazione delle componenti strutturali condivise.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item i \textbf{verificatori}, che identificano i membri che hanno eseguito attività di \term{\term{Verifica}} per quel documento; 
...
I verbali rappresentano i rendiconti di riunioni ufficiali interne, a cui partecipano solo i componenti del gruppo, ed esterne, a cui invece partecipano anche enti esterni al gruppo come impiegati dell'azienda proponente del \term{\term{Capitolato}} o i professori del corso. Ogni \term{\term{Verbale}} oltre alle sezioni precedentemente elencate deve presentare al suo interno i seguenti elementi nell'ordine in cui sono descritti:
\begin{enumerate}
  ...
  \item \textbf{Ordine del giorno}: riassume i punti principali che si intendono discutere durante l’incontro, predisposti in anticipo dal responsabile/i di progetto durante quel periodo. Questa sezione funge da guida per la conduzione del \term{\term{Verbale}}.
...
dove:
\begin{itemize}
    \item \textbf{VI/VE}, indica se la decisione fa riferimento ad un \term{\term{Verbale}} interno o esterno, seguendo le convenzioni di scrittura stabilite.
    \item \textbf{fase}, indica la fase di progetto a cui si riferisce;
    \item \textbf{Y}, indica il numero del \term{\term{Verbale}} in riferimento a quella revisione;
...
\end{itemize}
  ...
  \begin{itemize}
      ...
      \item il numero della issue GitHub di quella attività presente nel GitHub \term{\term{Project}}.
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item Il nome di ogni \term{\term{Verbale}} deve iniziare con \texttt{\term{\term{Verbale}}\_} , segue il tipo di \term{\term{Verbale}} \texttt{interno} o \texttt{esterno} e la data in cui si è svolto. Un esempio di nominazione corretta è \texttt{\term{\term{Verbale}}\_interno\_2025-11-05}.
    \item L'unica eccezione è nel caso in cui usare questo formato generi due verbali con lo stesso nome all'interno della stessa cartella. In tal caso è necessario aggiungere, dopo la data, una breve descrizione del contesto relativo al \term{\term{Verbale}}, sostituendo agli spazi il simbolo \textbf{underscore (\_)}. Ad esempio \texttt{\term{\term{Verbale}}\_2025-10-28\_incontro\_c8}.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{\term{\term{Rtb}}}, Requirements and \term{\term{Technology \term{Baseline}}};
        \item \textbf{\term{\term{Pb}}}, Product \term{\term{Baseline}}.
    \end{itemize}
    \item \textbf{Tipologie di documenti}:
    \begin{itemize}
        \item \textbf{AdR}, analisi dei requisiti;
        \item \textbf{NdP}, norme di progetto;
        \item \textbf{PdQ}, \term{\term{Piano Di Qualifica}};
        \item \textbf{PdP}, \term{\term{Piano Di Progetto}};
        \item \textbf{G}, glossario;
        \item \textbf{VI}, \term{\term{Verbale}} interno;
        \item \textbf{VE}, \term{\term{Verbale}} esterno.
    \end{itemize}
\end{itemize}
...
Ogni modifica ad un documento deve essere tracciabile all’interno dello stesso tramite lo \textbf{storico del documento}, riportando:
  \begin{itemize}
      ...
      \item il nome del verificatore che è tenuto ad eseguire la \term{\term{Verifica}}.
  \end{itemize}
...
Una volta completate le modifiche nel nuovo branch aperto, il membro incaricato procede ad aprire una \term{\term{Pull Request}} dal proprio branch di feature verso il branch \texttt{develop}. 
Nella fase di apertura della \term{\term{Pull Request}} deve essere compilato il form per la descrizione della \term{\term{Pull Request}} con le informazioni richieste. La \term{\term{Pull Request}} costituisce la richiesta formale per l'integrazione delle modifiche nel \term{\term{Repository}} del gruppo, dopo l'avvenuta approvazione delle modifiche apportate.

\paragraph{\term{\term{Verifica}}} Il verificatore esamina sia la correttezza formale sia sostanziale del documento.
    \begin{itemize}
        \item Se le modifiche risultano corrette, il verificatore approva la \term{\term{Pull Request}}.
        \item Se emergono incongruenze o problemi, il verificatore richiede delle modifiche aggiungendo dei commenti al codice.
    \end{itemize}
Le modifiche richieste devono essere applicate dall’autore originario della \term{\term{Pull Request}}, che procede aggiornando il branch \texttt{develop} fino alla completa risoluzione.
...
\paragraph{Approvazione e \term{\term{Pull Request}}} Una volta ottenuta l’approvazione del verificatore, la \term{\term{Pull Request}} viene sottoposta alla visione del responsabile, che effettua un ultima revisione formale. Solo il responsabile è autorizzato a procedere con il merge della \term{\term{Pull Request}} nel branch \texttt{develop}.
\\Il merge rappresenta l’integrazione definitiva del documento nella versione stabile del progetto.

\paragraph{Pubblicazione} Con la conclusione del merge, il documento aggiornato diventa parte integrante della \term{\term{Baseline}} di progetto. Il branch \texttt{main} costituisce, infatti, il riferimento ufficiale per la versione stabile e coerente dei documenti.
...
\begin{itemize}
    \item \textbf{GitHub}: piattaforma per il versionamento, la gestione dei \term{\term{Repository}}, delle issue, delle \term{\term{Pull Request}} e dei flussi di lavoro collaborativi mediante GitHub Projects e Teams. È inoltre il punto ufficiale di archiviazione degli artefatti approvati.
    \item \textbf{GitHub Actions}: utilizzate per automatizzare la compilazione dei documenti in \LaTeX{} e garantire la \term{\term{Validazione}} continua del \term{\term{Repository}}.
    \item \textbf{Branch protection rules}: utilizzate per impedire modifiche non autorizzate sul branch principale e garantire che ogni integrazione avvenga secondo le regole di \term{\term{Verifica}} previste dal gruppo.
\end{itemize}

\subsubsection{Implementazione di processo}
Le attività principali del processo di gestione della configurazione sono:
\begin{itemize}
    \item \textbf{identificazione della configurazione}, viene definito uno schema uniforme di versionamento e naming, per identificare ogni \term{\term{Componente}} del sistema;
    \item \textbf{controllo della configurazione}, metodologie per stabilire l'identificazione, l'approvazione o rifiuto, la \term{\term{Verifica}} e l'eventuale rilascio delle richieste di modifica; 
...
\begin{center}
    \texttt{\term{\term{Task}}/<descrizione delle modifiche>\_<documento soggetto alle modifiche>}
\end{center}
Dove:
\begin{itemize}
...
Al momento della creazione, la issue deve essere dotata di un titolo descrittivo e di una spiegazione chiara dell’obiettivo. Il responsabile assegna inoltre priorità, \term{\term{Milestone}}, iterazione (che corrisponde allo \term{\term{Sprint}} in cui l’attività deve essere completata) e l’assegnatario, selezionato in base al ruolo che il membro ricopre in quel momento. Label aggiuntive consentono una categorizzazione accurata del lavoro.\\ 
La pianificazione complessiva è supportata dalle GitHub \term{\term{Project}} Board, che permettono di consultare product \term{\term{Backlog}} e \term{\term{Sprint}} \term{\term{Backlog}} tramite viste personalizzate, semplificando la gestione dei carichi di lavoro e l’avanzamento delle attività.

\paragraph{Creazione della \term{\term{Pull Request}}}
Una volta completata l’attività descritta nella issue GitHub, il membro incaricato apre una \term{\term{Pull Request}} dal branch di feature al \texttt{branch develop}.\\ Ogni \term{\term{Pull Request}} deve utilizzare il template comune del gruppo, che contiene le sezioni necessarie per descrivere contesto, modifiche introdotte, issue collegate e controlli effettuati. La descrizione del template presente nel file \texttt{pull\_request\_template.md}, deve essere compilato e adattato ogni volta, così da velocizzare il lavoro di revisione e rendere uniformi tutte le richieste. Il membro deve indicare tramite il campo \texttt{Reviewers} il singolo membro incaricato del compito di verificare quella modifica.

\paragraph{\term{\term{Verifica}} e revisione della \term{\term{Pull Request}}}
All'apertura della \term{\term{Pull Request}} viene richiesta automaticamente la \term{\term{Verifica}} ai membri del team verificatori:
\begin{itemize}
    \item se la \term{\term{Verifica}} produce esito positivo, la \term{\term{Pull Request}} viene approvata dal verificatore incaricato;
    \item se sono presenti dei problemi o miglioramenti da applicare, il verificatore richiede delle modifiche, tramite l'opzione di GitHub \texttt{request changes}, aggiungendo un commento di quello che è il problema trovato e di una possibile soluzione se è presente. Successivamente la persona che ha aperto la \term{\term{Pull Request}} si impegna a eseguire le correzioni e a notificare il completamento dell'azione per avere una \term{\term{Verifica}} il prima possibile.
\end{itemize}

\paragraph{Integrazione nella \term{\term{Baseline}}}
Quando la \term{\term{Pull Request}} viene approvata dal verificatore, il responsabile esegue la revisione. Si impegna ad eseguire il merge nel branch \texttt{develop}. A questo punto, la modifica entra ufficialmente in \term{\term{Baseline}} e diventa parte del progetto.
...
\begin{itemize}
    ...
    \item le \term{\term{Milestone}} attive e completate;
    \item la cronologia delle \term{\term{Pull Request}};
...
L’accertamento della qualità consente di fornire evidenze oggettive sul fatto che i sistemi software sviluppati siano idonei allo scopo per cui sono stati richiesti e soddisfino le aspettative degli \term{\term{Stakeholder}} coinvolti.

\subsubsection{Implementazione di processo}
Il gruppo \textit{BitByBit} ha identificato come principale attività del processo di accertamento della qualità la redazione del documento di \textbf{\term{\term{Piano Di Qualifica}}}.
...
Per ciascuna metrica vengono inoltre individuati, nel \term{\term{Piano Di Qualifica}}, dei valori ottimali o soglie di riferimento, al fine di identificare tempestivamente eventuali comportamenti anomali o scostamenti rispetto agli obiettivi di qualità prefissati.\\
...
Nel \term{\term{Piano Di Qualifica}} vengono definite specifiche metriche di qualità di prodotto.
...
Nel \term{\term{Piano Di Qualifica}} vengono formalizzate le metriche di processo e i relativi valori ottimali stabiliti dal gruppo.
...
% ==== SEZIONE PROCESSO DI \term{\term{Verifica}} ====
\subsection{Processo di \term{\term{Verifica}}} \label{sec:processo-verifica}
Il processo di \term{\term{Verifica}} è l'insieme delle attività volte ad accertare che ogni prodotto del ciclo di vita (codice, documenti, \term{\term{Architettura}}) sia conforme ai requisiti e agli standard di qualità stabiliti.
...
Il gruppo \textit{BitByBit} integra la \term{\term{Verifica}} direttamente nel flusso di sviluppo quotidiano (Git Flow), adottando la politica del \textbf{Continuous Verification}.
Le regole operative sono le seguenti:
\begin{itemize}
    \item \textbf{Policy "No Verify, No Merge"}: nessun artefatto può essere integrato nel ramo di sviluppo principale (\texttt{develop}) se non ha superato con successo i controlli di \term{\term{Verifica}}.
    \item \textbf{\term{\term{Pull Request}} come Quality Gate}: ogni modifica deve essere proposta tramite \term{\term{Pull Request}}. In questa sede avviene la \term{\term{Verifica}} (automatica o manuale) e solo l'esito positivo autorizza il merge.
...
\subsubsection{\term{\term{Verifica}} della Documentazione}
...
\paragraph{Strumenti per la \term{\term{Verifica}} della documentazione}
Poiché il gruppo ha adottato la tecnica dell'\textbf{Ispezione} basata su checklist, la \term{\term{Verifica}} della documentazione non richiede strumenti di automazione complessi, ma si avvale delle piattaforme di sviluppo e collaborazione:
\begin{itemize}
    \item \textbf{GitHub}: è lo strumento principale. L'interfaccia delle \term{\term{Pull Request}} permette ai verificatori di commentare righe specifiche del documento, richiedere modifiche e approvare il lavoro solo quando la checklist di \term{\term{Verifica}} è soddisfatta.
...
\subsubsection{\term{\term{Verifica}} del Codice Software}
La \term{\term{Verifica}} del software si articola in due momenti distinti: l'analisi del codice sorgente (statica) e l'esecuzione dei test (dinamica).
...
\begin{itemize}
    ...
    \item potenziali \term{\term{Bug}} logici o di sicurezza (es. variabili non utilizzate, cicli infiniti);
    \item calcolo delle metriche di complessità.
\end{itemize}

\paragraph{Analisi Dinamica (Testing)}
L'analisi dinamica \term{\term{Verifica}} il comportamento del software tramite la sua esecuzione.
...
\begin{enumerate}
    \item \textbf{Test di Unità}: \term{\term{Verifica}} delle singole componenti (funzioni/classi) in isolamento.
    \begin{itemize}
    ...
    \item \textbf{\term{\term{Test Di Integrazione}}}: \term{\term{Verifica}} della collaborazione tra unità già testate.
    \begin{itemize}
    ...
    \item \textbf{Test di Sistema}: \term{\term{Verifica}} del sistema completo rispetto ai requisiti funzionali e prestazionali mappati nell'Analisi dei Requisiti.
    ...
    \item \textbf{Test di Accettazione}: Verifiche formali finali, condotte da utenti o \term{\term{Stakeholder}}, per confermare che un software soddisfi i requisiti aziendali e funzionali prima del rilascio in produzione.
\end{enumerate}

\paragraph{Strumenti per la \term{\term{Verifica}} del codice}
La definizione puntuale degli strumenti tecnici per la \term{\term{Verifica}} del codice (es. framework di unit testing, linter specifici per linguaggio, strumenti di analisi statica e coverage) sarà effettuata e dettagliata durante lo svolgimento della \textbf{Product \term{\term{Baseline}}} (\term{\term{Pb}}), in coerenza con le tecnologie che verranno consolidate in quella fase.

\subsubsection{Classificazione dei Test}
Per garantire l'ordine nel \textbf{\term{\term{Piano Di Qualifica}}}, il gruppo adotta una nomenclatura univoca per il tracciamento:
\begin{center}
    \texttt{T-[ID]-[Tipo]}
\end{center}
...
% ==== SEZIONE PROCESSO DI \term{\term{Validazione}} ====
\subsection{Processo di \term{\term{Validazione}}}
Il processo di \term{\term{Validazione}} costituisce l'ultimo livello di controllo qualitativo prima del rilascio formale del prodotto.
A differenza della \term{\term{Verifica}}, che si configura come un controllo tecnico continuo sul codice, la \term{\term{Validazione}} ha lo scopo di accertare che il sistema software realizzato dal gruppo \textit{BitByBit} sia pienamente conforme alle aspettative e ai bisogni reali dell'azienda proponente \textbf{Miriade}.
...
\subsubsection{Tracciamento come strumento di \term{\term{Validazione}}}
...
L'attività prevede di mappare univocamente ogni \term{\term{Requisito}} (funzionale, di qualità o vincolo) ai relativi test di \term{\term{Validazione}}.
Questo approccio metodologico garantisce che non venga sviluppato codice non richiesto e, soprattutto, che ogni \term{\term{Requisito}} concordato con \textbf{Miriade} sia verificabile tramite un test specifico. Un \term{\term{Requisito}} privo di tracciamento verso un test è considerato non validabile e quindi non accettabile.

\subsubsection{Esecuzione dei test di accettazione}
La \term{\term{Validazione}} si concretizza con l'esecuzione dei \textbf{Test di Accettazione} (UAT - User Acceptance Testing).
In questa fase il gruppo non si limita a verificare l'assenza di errori, ma simula scenari di utilizzo reali (User Scenarios) per valutare il comportamento del sistema dal punto di vista dell'\term{\term{Utente}} finale.
...
\subsubsection{Strumenti a supporto della \term{\term{Validazione}}}
Analogamente alla \term{\term{Verifica}}, anche il processo di \term{\term{Validazione}} si avvale di strumenti specifici per gestire la complessità del tracciamento e l'esecuzione dei test.
...
\begin{itemize}
    \item \textbf{Ambiente di \term{\term{Deployment}}}: i test verranno eseguiti su un'istanza del software deployata in un ambiente il più possibile simile a quello di produzione, accessibile tramite Browser Web standard (Chrome/Firefox).
    \item \textbf{GitHub Issues}: qualsiasi anomalia o comportamento inatteso riscontrato durante la \term{\term{Validazione}} (test fallito) verrà tracciato aprendo una Issue dedicata su GitHub, etichettata specificamente come \term{\term{Bug}} di \term{\term{Validazione}} per distinguerla dai difetti tecnici di sviluppo.
\end{itemize}
...
Sul branch principale (\texttt{main}) solo i membri del team dei Responsabili possono eseguire push o merge, mentre tutti gli altri membri propongono modifiche tramite \term{\term{Pull Request}}. La funzione \texttt{CODEOWNERS} assegna automaticamente ai Verificatori la responsabilità di revisione dei documenti, garantendo che tutte le modifiche rispettino il workflow e i controlli qualitativi previsti.\\
...
\begin{itemize}
    ...
    \item \textbf{Revisione e valutazione}, perché un prodotto entri in \term{\term{Baseline}}, le modifiche apportate devono essere prima verificate da un verificatore e successivamente revisionate dal responsabile. Quest’ultimo accerta che i prodotti software e i piani risultino conformi ai requisiti stabiliti, valutando l’esito dei compiti completati e garantendo qualità e coerenza con gli obiettivi del processo.
    \item \textbf{Chiusura}, al completamento delle attività e dei prodotti previsti, si \term{\term{Verifica}} che i criteri concordati con l’acquirente siano soddisfatti. Il responsabile controlla la completezza dei risultati e dei report.
\end{itemize}
...
\hline
\rowcolor{white}
\centering Responsabile & È la figura che nel momento governa il team. Identifica le attività che sono state eseguite nello \term{\term{Sprint}} terminato e quelle che non sono state portate a termine. Durante l'evento di \term{\term{Sprint}} Planning stabilisce ed inserisce nello \term{\term{Sprint}} \term{\term{Backlog}} le attività da eseguire nello \term{\term{Sprint}} successivo, individuando i costi, i rischi e i membri del gruppo in base ai loro ruoli. Rappresenta il gruppo all'esterno, interfacciandosi con gli enti esterni quali i professori e l'azienda proponente.    \\
\hline
\rowcolor{gray!20}
\centering Amministratore & È l'amministratore di sistema (Sys Admin), si occupa della gestione dell'infrastruttura utilizzata per eseguire le attività dei processi di ciclo di vita. Ha il compito di inserire nell'infrastruttura gli strumenti e le tecnologie per l'attuazione del \term{\term{Way Of Working}}. Deve accertarsi che l'infrastruttura si trovi in uno stato stabile e quindi risolvere eventuali problematiche che possono derivarne. Deve redigere il presente documento.\\
\hline
\rowcolor{white}
...
\hline
\rowcolor{white}
\centering Programmatore & È la persona cui spetta il compito di tradurre le unità di design in codice funzionante. In concomitanza con il codice deve anche implementare i test automatici per la \term{\term{Verifica}} del corretto funzionamento del codice.\\
\hline
\rowcolor{gray!20}
...
L’organizzazione interna del gruppo (si veda sezione 4.1) è stata implementata tramite la funzione nativa di GitHub Teams, che consente di suddividere i membri in sotto–gruppi corrispondenti ai ruoli previsti dal progetto (si veda Sezione 4.1.3). Ogni ruolo forma un team composto da uno a più membri, così da distribuire responsabilità e carico di \term{\term{Verifica}}.\\
Le figure che interessano le \term{\term{Pull Request}} sono principalmente due:
\begin{itemize}
    \item verificatori, scelti tra i membri del team con responsabilità di \term{\term{Verifica}} in quello \term{\term{Sprint}}, devono controllare la correttezza formale delle modifiche;
...
\begin{itemize}
    \item \textbf{Interne}: si svolgono tra i soli membri del gruppo e hanno luogo principalmente su Discord. Ogni \term{\term{Sprint}} ha una durata di due settimane e prevede tre incontri:
    \begin{itemize}
        \item uno a fine \term{\term{Sprint}}, comprendente \term{\term{Sprint}} Review \term{\term{Sprint}} Retrospective e \term{\term{Sprint}} Planning. In questa sede il \textbf{Responsabile} \term{\term{Verifica}} lo stato delle attività concluse, analizza eventuali problemi riscontrati e pianifica il lavoro del successivo \term{\term{Sprint}};
        \item due durante lo svolgimento dello \term{\term{Sprint}}, in cui vengono trattati i temi dell'ordine del giorno e ciascun membro espone le attività completate, quelle ancora in corso e le difficoltà incontrate.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Redattore}: responsabile della stesura del \term{\term{Verbale}} e del caricamento nel \term{\term{Repository}}.
    \end{itemize}
...
\end{itemize}
L'esito di ogni incontro, interno o esterno, deve essere registrato mediante la redazione di un \term{\term{Verbale}}, secondo le modalità previste dal processo di documentazione. 
...
\hline
\rowcolor{white}
\centering Google Meet & Piattaforma sincrona per videoconferenze utilizzata negli incontri con l’azienda proponente, in quanto standard scelto dal \term{\term{Committente}}.\\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{gray!20}
\centering GitHub & Piattaforma cloud-based di hosting per \term{\term{Repository}} Git, utilizzata per la gestione collaborativa del codice. Scelta per le funzionalità integrate (issue tracking, \term{\term{Pull Request}}, branching rules, code review) che supportano il processo di sviluppo.\\
\hline
\rowcolor{white}
...
\hline
\rowcolor{gray!20}
\centering Google Drive & Spazio di archiviazione cloud utilizzato dal gruppo per raccogliere documenti correlati, appunti delle riunioni, fogli di calcolo in cui il gruppo registra le informazioni informali riguardanti i periodi di \term{\term{Sprint}}. Scelto per la centralizzazione dei contenuti, l’accessibilità da qualsiasi dispositivo e la possibilità di modifica concorrente.\\
\hline
\rowcolor{white}
\centering Google Sheets & Foglio di calcolo online utilizzato per i dati riguardanti i singoli \term{\term{Sprint}} e rappresentarli in appositi grafici per una visualizzazione migliore. Scelto per la collaborazione sincrona e la facilità di aggiornamento in tempo reale.\\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
\centering Google Presentazioni & Strumento per la realizzazione di presentazioni utilizzate nelle esposizioni verso professori e \term{\term{Stakeholder}}. Scelto per la capacità di collaborazione e per i template professionali disponibili.\\
\hline
\rowcolor{gray!20}
\centering Script Python & Script sviluppati per automatizzare attività ricorrenti nel processo, come operazioni di generazione automatica o \term{\term{Validazione}}. Scelti per la flessibilità del linguaggio e la facilità di integrazione con altre tecnologie.\\
\hline
\rowcolor{white}
...
\paragraph{Git}
Per garantire coerenza operativa tra tutti i membri del team, è previsto un insieme minimo di configurazioni locali che ciascun \term{\term{Componente}} deve applicare al proprio ambiente Git.
\begin{itemize}
    ...
    \item Il membro deve assicurarsi che Git sia correttamente installato nel proprio ambiente, e che sia possibile accedere alla \term{\term{Repository}} remota tramite le chiavi SSH configurate.
\end{itemize}

\paragraph{GitHub} 
La configurazione del \term{\term{Repository}} è definita per supportare il workflow adottato e garantire qualità, tracciabilità e sicurezza delle modifiche.\\
Al suo interno è possibile trovare varie directories, di seguito verranno descritte le più rilevanti.
\begin{itemize}
    \item \textbf{.github} contiene la configurazione delle automazioni del progetto. Include il file \texttt{CODEOWNERS}, che definisce i responsabili delle diverse directory e consente a GitHub di assegnare automaticamente i revisori per le \term{\term{Pull Request}}. Contiene inoltre il file \texttt{latex\_template.txt}, che rappresenta il template comune a tutti i documenti del gruppo BitByBit, e \texttt{template\_pull\_request.md}, il modello utilizzato per la creazione automatica delle \term{\term{Pull Request}}.
    \begin{itemize}
        \item \textbf{.github/workflows} contiene i workflow GitHub Actions dedicati all’integrazione continua e all’automazione delle attività del \term{\term{Repository}}.
    ...
    \item \textbf{resources} funge da archivio centralizzato delle risorse condivise, includendo immagini, loghi e altri materiali comuni utilizzati nel \term{\term{Repository}}.
...
\begin{itemize}
    ...
    \item \textbf{Formula:} $\text{\# di linee di codice per una \term{\term{Componente}} software}$
    \item \textbf{Descrizione:} misura la dimensione di un \term{\term{Componente}} software. Più è grande una \term{\term{Componente}} software maggiore sarà la possibilità di sviluppare errori.
\end{itemize}
...
\paragraph{\term{\term{Bug}} Rate per Test Case}
\begin{itemize}
    \item \textbf{Codice:} \texttt{M-PD10}
    \item \textbf{Formula:} $\frac{\text{\# \term{\term{Bug}} individuati}}{\text{\# test case eseguiti}}\cdot 100$
...
\begin{itemize}
    ...
    \item \textbf{Formula:} $PV=(\text{BAC})\cdot(\text{\% di lavoro pianificata nello \term{\term{Sprint}}})$
    \item \textbf{Descrizione:} rappresenta il preventivo per i costi del lavoro che si intende eseguire durante uno \term{\term{Sprint}} di progetto.
\end{itemize}

\paragraph{Actual Cost (AC)}
\begin{itemize}
    \item \textbf{Codice:} \texttt{M-PC3}
    \item \textbf{Formula:} $AC=\text{costo effettivo sostenuto in uno \term{\term{Sprint}}}$
    \item \textbf{Descrizione:} rappresenta i costi effettivamente sostenuti per completare il lavoro svolto durante uno \term{\term{Sprint}} di progetto.
\end{itemize}
...
\subsubsection{\term{\term{Verifica}}}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{inferiore a 80} sono difficili da leggere per chi ha la \term{\term{Licenza}} elementare
        \item \textbf{inferiore a 60} sono difficili da leggere per chi ha la \term{\term{Licenza}} media
...
\begin{itemize}
    ...
    \item \textbf{Descrizione:} misura la percentuale di metriche che sono state soddisfatte durante uno \term{\term{Sprint}}. Una metrica viene considerata soddisfatta quando presenta un valore accettabile.
\end{itemize}

\subsubsection{Gestione dei processi}
\paragraph{\term{\term{Sprint}} Commitment Reliability (SCR)}
\begin{itemize}
    \item \textbf{Codice:} \texttt{M-PC14}
    \item \textbf{Formula:} $\frac{\text{\# issue completate nello \term{\term{Sprint}}}}{\text{\# issue pianificate}} * 100$
    \item \textbf{Descrizione:} misura la capacità del team di rispettare gli impegni pianificati per lo \term{\term{Sprint}}.
