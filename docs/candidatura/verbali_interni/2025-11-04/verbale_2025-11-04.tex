...
\begin{itemize}
    ...
    \item \textbf{Un singolo \term{Verbale} interno:} il gruppo evidenzia il fatto che siano stati redatti 3 Verbali interni. La confusione probabilmente nasce dal fatto che nel branch develop sia presente un singolo \term{Verbale} interno, mentre nel main sono presenti 3 Verbali interni. 
    \item \textbf{Tutti i Verbali, anche quelli esterni, devono confluire in azioni tracciabili:} riconosciamo l'assenza di una mappatura tra Verbali e azioni concrete svolte dai membri del gruppo. D'ora in avanti ciascun \term{Verbale} indicherà esplicitamente le azioni necessarie da esso emerse, con annessi i membri del gruppo assegnati e la data prevista di completamento. Ogni azione nel \term{Verbale} punterà alla corrispondente issue sul \term{Repository} GitHub.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Organizzazione non ottimale del lavoro sulla \term{Repository} remota:} in alcuni casi dei compiti non erano associati a delle issue specifiche; in altri casi le issue rimanevano aperte nonostante i relativi compiti fossero già stati completati. D'ora in avanti ogni compito dovrà avere una issue assegnata immediatamente a un membro del gruppo. Al completamento del lavoro, la issue corrispondente dovrà essere chiusa.
...
\begin{center}
    ...
    \begin{tabular}{|p{0.35\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|}
        \hline
        \rowcolor{gray!60} 
        \textbf{\term{Task}} & \textbf{Assegnatario} & \textbf{Scadenza} & \textbf{Link GitHub} \\
        \hline
        \rowcolor{white}
